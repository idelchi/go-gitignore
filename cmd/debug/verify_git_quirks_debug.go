...
func testSandwichWildcards(tempDir string) {
	...
	os.MkdirAll(filepath.Join(tempDir, "src/node_modules_test/pkg"), 0755)
	os.WriteFile(filepath.Join(tempDir, "src/node_modules_test/file.js"), []byte("test"), 0644)
...
func testContentsOnly(tempDir string) {
	...
	os.MkdirAll(filepath.Join(tempDir, "build/sub"), 0755)
	os.WriteFile(filepath.Join(tempDir, "build/file.txt"), []byte("test"), 0644)
