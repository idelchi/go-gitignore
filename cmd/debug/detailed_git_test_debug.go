...
func testDotNegationDetailed(tempDir string) {
	...
	os.WriteFile(filepath.Join(tempDir, "file1.txt"), []byte("test"), 0o644)
	os.WriteFile(filepath.Join(tempDir, "file2.txt"), []byte("test"), 0o644)
...
func testContentsOnlyDetailed(tempDir string) {
	...
	os.MkdirAll(filepath.Join(buildDir, "subdir"), 0o755)
	os.WriteFile(filepath.Join(buildDir, "file.txt"), []byte("content"), 0o644)
	os.WriteFile(filepath.Join(buildDir, "subdir", "nested.txt"), []byte("content"), 0o644)
...
func testDotfiles(tempDir string) {
	...
	// Create dotfiles
	os.WriteFile(filepath.Join(tempDir, ".hidden"), []byte("content"), 0o644)
	os.WriteFile(filepath.Join(tempDir, "visible.txt"), []byte("content"), 0o644)
